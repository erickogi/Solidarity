...
class Validator {
    companion object {
        ...
        private val PIN_POLICY= "Pin must be four digits"
        ...
        fun passwordsMatch(edt_password: TextInputEditText, edt_confirm_password: TextInputEditText): Boolean {
            ...
            if (!st) {
                setError(edt_password, "Pin do not match")
                setError(edt_confirm_password, "Pin do not match")
            }
            return st
        }
        ...
        fun isValidPin(data: Any, updateUI: Boolean = true): Boolean {
            val str = getText(data)
            var valid = true
            // Password policy check
            // Password should be minimum minimum 8 characters long
